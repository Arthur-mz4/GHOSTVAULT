  ...
  const handleNavRequest = (req) => {
    try {
      ...
      if (settings.promptScanOnLinks) {
        // Start scan in background but allow navigation unless risk is detected
        scanUrl(target, settings)
          .then(result => {
            if (!result.safe) {
              setModal({ visible: true, pendingUrl: target, result });
          ...
          .catch(err => {
            setModal({ 
              visible: true, 
              pendingUrl: target, 
              result: { safe: false, reason: 'Scan failed: ' + (err?.message || err) }
            });
            if (webviewRef.current) {
              webviewRef.current.stopLoading();
            }
          });
        return true; // Allow initial navigation
      }
        ...
        <View style={{ flex: 1 }}>
          <WebView
            ...
            onNavigationStateChange={(navState) => { 
              if (navState.url && navState.url !== 'about:blank') setUrl(navState.url);
...
const styles = StyleSheet.create({
  ...
  backButton: { width: 40, height: 40, justifyContent: 'center', alignItems: 'center', borderRadius: 8, marginRight: 8 },
