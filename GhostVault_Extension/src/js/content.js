...
async function decodePassword(obj, mp) {
  try {
    if (!obj || !mp) {
      console.warn('Missing parameters for password decoding');
      return null;
    }
    
    if (!obj.salt || !obj.iv || !obj.data) {
      console.warn('Invalid encrypted password object');
      return null;
    }
    
    const key = await deriveKey(mp, new Uint8Array(obj.salt));
    ...
    console.error('Password decoding failed:', error.message);
    return null;
  }
}
...
async function getMasterPassword() {
  return new Promise((resolve) => {
    ...
    chrome.runtime.sendMessage({ action: 'requestMasterPassword' })
      .then(response => {
...
window.addEventListener('load', async () => {
  ...
  chrome.storage.sync.get('logins', async data => {
    ...
    if (savedLogin) {
      ...
      let decodedPass = '';
      try {
        decodedPass = await decodePassword(savedLogin.pass, mp);
      } catch {
        decodedPass = '';
      }
...
function showSavePrompt(site, username, password) {
  ...
  modal.innerHTML = `
    <div style="background:#fff;padding:24px 18px;border-radius:10px;box-shadow:0 2px 12px rgba(0,0,0,0.2);min-width:260px;max-width:90vw;text-align:center;">
      <h2 style="margin-bottom:12px;font-size:1.2rem;color:#00796b;">Save password to GhostVault?</h2>
      <div style="margin-bottom:10px;font-size:1rem;">For <b>${site}</b><br>Username: <b>${username}</b></div>
      <div style="margin-top:10px;">
        <button id="ghostvaultSaveBtn" style="background:#00796b;color:#fff;border:none;padding:8px 18px;border-radius:5px;font-size:1rem;margin-right:8px;">Save</button>
        <button id="ghostvaultNeverBtn" style="background:#eee;color:#333;border:none;padding:8px 18px;border-radius:5px;font-size:1rem;margin-right:8px;">Never for this site</button>
        <button id="ghostvaultCancelBtn" style="background:#eee;color:#333;border:none;padding:8px 18px;border-radius:5px;font-size:1rem;">Cancel</button>
      </div>
    </div>
  `;
  ...
  document.getElementById('ghostvaultSaveBtn').onclick = () => {
    ...
    .catch(error => {
      if (error && error.message && error.message.includes('Could not establish connection. Receiving end does not exist.')) {
        // Fallback: Save directly to IndexedDB from content script
        if (typeof GhostVaultDB !== 'undefined' && GhostVaultDB.saveLogin) {
          GhostVaultDB.saveLogin(loginData).then(() => {
            // Optionally show a non-blocking notification/modal, or just log
            if (!document.getElementById('ghostvaultLocalSaveNotice')) {
              const notice = document.createElement('div');
              notice.id = 'ghostvaultLocalSaveNotice';
              notice.textContent = 'Login saved locally (background unavailable).';
              notice.style.position = 'fixed';
              notice.style.bottom = '20px';
              notice.style.right = '20px';
              notice.style.background = '#e0ffe0';
              notice.style.color = '#080';
    ...
    chrome.storage.sync.get({ neverSaveSites: [] }, data => {
      ...
      if (!neverSaveSites.includes(site)) {
        ...
        chrome.storage.sync.set({ neverSaveSites }, () => {
          // Notify the popup to refresh its UI
          try {
            chrome.runtime.sendMessage({ action: 'refreshUI' });
          } catch (e) {
            console.log('Could not notify popup to refresh UI (might not be open)');
